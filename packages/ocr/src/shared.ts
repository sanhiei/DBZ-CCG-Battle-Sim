...
export const REGIONS = {
  ...
  textBox: { x0: 0.13, y0: 0.615, x1: 0.96, y1: 0.87 } satisfies Rect,
  ...
  powerColumn: { x0: 0.71, y0: 0.11, x1: 0.995, y1: 0.875 } satisfies Rect,
  pur: { x0: 0.04, y0: 0.63, x1: 0.18, y1: 0.8 } satisfies Rect,
  level: { x0: 0.09, y0: 0.02, x1: 0.24, y1: 0.115 } satisfies Rect,
  powerText: { x0: 0.19, y0: 0.6, x1: 0.74, y1: 0.805 } satisfies Rect,
};
...
export function px(rect: Rect, w: number, h: number) {
...
export async function regionBuffer(
  path: string,
  rect: Rect,
  w: number,
  h: number,
  opts: { mode?: Preprocess } = {},
): Promise<Buffer> {
...
export function guessType(typeLineText: string): { type: string | null; conf: 'match' | 'none' } {
...
export function parseLadder(text: string): { ratings: Array<number | 'Z'>; suspect: boolean; dropped: number } {
  const lines = text
    .split('\n')
 ...
 */
function longestIncreasing(ratings: Array<number | 'Z'>): Array<number | 'Z'> {
  const zs = ratings.filter((r) => r === 'Z');
  const nums = ratings.filter((r): r is number => typeof r === 'number');
  if (nums.length === 0) return ratings;
