...
const mp=names.map(n=>({n,lv:lv(n)})).filter(x=>x.lv.length===3)[3];
...
ws.on('message',raw=>{const m=JSON.parse(String(raw));
  if(m.kind==='error')console.log('OPP ERR:',m.message);
  if(m.kind==='state'&&m.state.pendingPrompt) console.log('OPP sees prompt:',m.state.pendingPrompt.type,'for seat',m.state.pendingPrompt.playerIdx);
});
ws.send(JSON.stringify({kind:'join',roomCode:'FIGHT',playerName:'Opponent'}));
...
console.log('seat 1 ready in FIGHT with', mp.n);
